...
- **綠色**：HCI 關鍵字，例如 `人機互動`、`眼動追蹤`、`無障礙`、`虛擬實境`。
  這些同時是相關度分數的來源，看得出它為什麼被判成 HCI 相關。
- **灰色**：領域標籤，例如 `醫療`、`機器人`、`能源`、`量子`、`法律政策`。
  不算分，只是讓非 HCI 的職缺也一眼看得出在做什麼。

對照表在 `scraper/topics.yml`，格式是「正則: 中文標籤」，想改用詞直接改那裡。
